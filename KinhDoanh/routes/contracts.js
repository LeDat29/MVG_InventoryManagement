...
router.get('/', requirePermission('contract_read'), catchAsync(async (req, res) => {
    ...
    let rawContractsRows = [];
    try {
        const [rawContracts] = await pool.execute(`
            SELECT c.*, cust.name as company_name, cust.representative_name as customer_name,
        ...
        `, queryParams);
        rawContractsRows = Array.isArray(rawContracts) ? rawContracts : [];
    } catch (dbErr) {
        logger.error('DB error fetching contracts list:', dbErr);
        return res.status(500).json({ success: false, message: 'Lỗi khi truy vấn danh sách hợp đồng' });
    }

    // Normalize contracts to ensure frontend never receives unexpected types
    const contracts = rawContractsRows.map(row => {
        // Defensive normalization
        const normalized = {
            id: row.id ?? null,
            contract_number: row.contract_number ?? '',
            contract_title: row.description ?? '',
            ...
            rental_price: row.total_value !== undefined && row.total_value !== null ? Number(row.total_value) : 0,
    ...
    let totalResult;
    try {
        [totalResult] = await pool.execute(`
        ...
        logger.error('DB error counting contracts:', dbErr);
        return res.status(500).json({ success: false, message: 'Lỗi khi đếm hợp đồng' });
    }

    const total = (totalResult && totalResult[0] && totalResult[0].total) ? totalResult[0].total : 0;
...
router.get('/debug', requirePermission('contract_read'), catchAsync(async (req, res) => {
    const pool = mysqlPool();
    ...
    // Build normalized view (same logic as main endpoint)
    ...
    const anomalies = normalized.filter(n => !n.id || !n.contract_number || n.rental_price === 0 || !n.start_date || !n.end_date);

    logger.info('Contracts debug route accessed', { limit, totalSample: rows.length, anomalies: anomalies.length });
