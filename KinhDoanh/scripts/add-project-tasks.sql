...
-- Insert sample tasks (optional - for demo)
...
SELECT 
    id as project_id,
    'fire_safety' as task_type,
    'Kiểm tra hệ thống phòng cháy chữa cháy' as title,
    'Kiểm tra thiết bị PCCC, bình cứu hỏa, hệ thống báo cháy, đường thoát hiểm' as description,
    'quarterly' as frequency,
    CURDATE() as start_date,
    DATE_ADD(CURDATE(), INTERVAL 90 DAY) as due_date,
    DATE_ADD(CURDATE(), INTERVAL 90 DAY) as next_due_date,
    TRUE as is_recurring,
    'high' as priority,
    'pending' as status,
    1 as created_by
FROM projects 
WHERE id IN (SELECT id FROM projects ORDER BY id LIMIT 3);
...
SELECT 
    id as project_id,
    'security' as task_type,
    'Kiểm tra hệ thống an ninh' as title,
    'Kiểm tra camera, cửa ra vào, hệ thống khóa, tuần tra an ninh' as description,
    'monthly' as frequency,
    CURDATE() as start_date,
    DATE_ADD(CURDATE(), INTERVAL 30 DAY) as due_date,
    DATE_ADD(CURDATE(), INTERVAL 30 DAY) as next_due_date,
    TRUE as is_recurring,
    'high' as priority,
    'pending' as status,
    1 as created_by
...
SELECT 
    ...
    'Bảo trì hệ thống điện' as title,
    'Kiểm tra tủ điện, dây cáp, đèn chiếu sáng, thiết bị điện' as description,
    'biweekly' as frequency,
    CURDATE() as start_date,
    DATE_ADD(CURDATE(), INTERVAL 14 DAY) as due_date,
    DATE_ADD(CURDATE(), INTERVAL 14 DAY) as next_due_date,
    TRUE as is_recurring,
    'medium' as priority,
    'pending' as status,
    1 as created_by
